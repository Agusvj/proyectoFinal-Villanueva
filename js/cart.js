...
// Funciones y botones del carrito
...
/**
 * Suma en cantidad a un item del carrito
 * @param {number} id 
 */
...
/**
 * Resta en cantidad a un item del carrito, si es 0 la elimina
 * @param {number} id 
 */
...
const renderProductosC = () => {
	...
	for (let producto of productos_cart) {
		...
		cartBuy.innerHTML = `<img src="imgs/${producto.imagen}" class="img-cart ps-1" alt="${producto.nombre}"/>
												 ...
												 <button class="botonSumaResta" id="restarProducto-${producto.id}">
												 	-
												 </button>
												 <p class="card-text goldie pe-1 m-0">${producto.cantidad}</p>
												 <button class="botonSumaResta" id="sumarProducto-${producto.id}">
												 	+
												 </button>
